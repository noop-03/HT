...
class DBHelper {
  ...
  Future<List<Workout>> getAllWorkouts() async {
    final database = await db;
    final res = await database.query('workouts', orderBy: 'date DESC, id DESC');
    return res.map((e) => Workout.fromMap(e)).toList();
  }
  ...
  Future<void> updateSet(WorkoutSet s) async {
    final database = await db;
    await database.update('sets', s.toMap(withId: true),
...
class WorkoutProvider extends ChangeNotifier {
  ...
  List<Workout> workouts = []; // 선택 날짜 운동
  List<Workout> allWorkouts = []; // 전체 운동 기록
  ...
  Future<void> loadAllWorkouts() async {
    allWorkouts = await db.getAllWorkouts();
    notifyListeners();
  }
...
// -----------------------------
// MainScreen 개선 (CircularProgress)
// -----------------------------
    ...
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton(
        ...
        onPressed: () {
          Navigator.of(context).push(MaterialPageRoute(
              builder: (context) =>
                  WriteScreen(initialDate: provider.selectedDate)));
        },
      ),
    );
  }
                      ...
                      children: [
                        Expanded(
                          child: TextFormField(
                            ...
                            validator: (v) {
                              if (v == null) return '숫자 입력';
                              final n = int.tryParse(v);
                              if (n == null || n <= 0) return '자연수 입력';
                              return null;
                            },
                          ),
                        ),
...
class _WorkoutDetailScreenState extends State<WorkoutDetailScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final progress = provider.progressForWorkout(widget.workout.id);
    final progressPercent = (progress * 100).round();
    return Scaffold(
      appBar: AppBar(
...
class StatsScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    for (var w in provider.allWorkouts) {
    ...
    for (int i = 0; i < entries.length; i++) {
      barGroups.add(BarChartGroupData(
        x: i,
        barRods: [
          BarChartRodData(toY: entries[i].value.toDouble(), width: 16, color: Colors.blue),
        ],
    ...
    return Scaffold(
      ...
      body: Padding(
        ...
        child: barGroups.isEmpty
            ...
            : Column(
          children: [
            Text("총 ${provider.allWorkouts.length}회 운동 기록됨",
...
class _CalendarScreenState extends State<CalendarScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    for (var w in provider.allWorkouts) {
      ...
      workoutEvents[key] = [...(workoutEvents[key] ?? []), w];
    }

    List<Workout> getWorkoutsForDay(DateTime day) {
      final key = DateTime(day.year, day.month, day.day);
      return workoutEvents[key] ?? [];
    }
        ...
        children: [
          ...
          Expanded(
            child: Builder(builder: (context) {
              final list = getWorkoutsForDay(_selectedDay ?? DateTime.now());
              if (list.isEmpty) {
                return const Center(child: Text('해당 날짜의 운동이 없습니다.'));
              }
              return ListView(
                children: list
                    .map((w) => ListTile(
