...
class DBHelper {
  ...
  Future<List<Workout>> getWorkoutsByDate(String date) async {
    final database = await db;
    final res =
    await database.query('workouts', where: 'date = ?', whereArgs: [date], orderBy: 'id DESC');
    return res.map((e) => Workout.fromMap(e)).toList();
  }

  Future<List<WorkoutSet>> getSetsByWorkout(int workoutId) async {
    final database = await db;
    final res = await database.query('sets', where: 'workoutId = ?', whereArgs: [workoutId], orderBy: 'setIndex ASC');
...
class WorkoutProvider extends ChangeNotifier {
  ...
  WorkoutProvider() {
    // load asynchronously after construction
...
// -----------------------------
// Write Screen: add workouts for a date
// -----------------------------
                    ...
                    ElevatedButton(
                      ...
                      onPressed: () async {
                        if (_formKey.currentState!.validate()) {
                          ...
                          Navigator.of(context).pop(); // 돌아가서 갱신된 목록 확인
                        }
                      },
                    )
                  ],
...
// -----------------------------
// Workout detail: show sets with checkboxes
// -----------------------------
...
class _WorkoutDetailScreenState extends State<WorkoutDetailScreen> {
  ...
  void didChangeDependencies() {
    ...
    provider = Provider.of<WorkoutProvider>(context, listen: false); //시발 대체 뭐가 문제노
    if (!loaded) {
      _ensureSets();
    }
  }
